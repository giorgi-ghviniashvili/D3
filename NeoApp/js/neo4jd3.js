function Neo4jD3(_selector, _options) {
    ...
    function appendNameToNode(node) {
        node.each(function(element){
          ...
          var wordCount = nameSplitted.length;

          // so much hard code :((
          if (wordCount >= 3) {
            if (nameSplitted[0].length >= 8){
              nameSplitted = nameSplitted.slice(0, 1);
              nameSplitted[0] = nameSplitted[0].substr(0, 4) + "...";
            }
            else if (nameSplitted[1].length >= 9) {
              nameSplitted = nameSplitted.slice(0, 2);
              nameSplitted[1] = nameSplitted[1].substr(0, 5) + "...";
            }
            else if (nameSplitted[2].length >= 8) {
              nameSplitted = nameSplitted.slice(0, 3);
              nameSplitted[2] = nameSplitted[2].substr(0, 4) + "...";
            }
            else{
              nameSplitted = nameSplitted.slice(0, 3);
              nameSplitted[2] = nameSplitted[2] + "...";
            }
          }
          else if (wordCount == 2){
            if (nameSplitted[0].length >= 8){
            ...
            else if (nameSplitted[1].length >= 8) {
              nameSplitted = nameSplitted.slice(0, 2);
              nameSplitted[1] = nameSplitted[1].substr(0, 4) + "...";
            }
            else {
              nameSplitted = nameSplitted.slice(0, 2);
            }
    ...
    function appendNode() {
        return node.enter()
                   ...
                   .on('dblclick', function(d) {
                       // stick the nodes to their centers' coordinates
                       d.fx = d.x;
                       d.fy = d.y;
                       currentNodeDbCliked = d;
                       if (typeof options.onNodeDoubleClick === 'function') {
                          // save the id of the doubleCliked node
            ...
            result.data.forEach(function(data) {
                data.graph.nodes.forEach(function(node) {
                    if (!contains(graph.nodes, node.id)) {
                        if (currentNodeDbCliked) {
                          if (currentNodeDbCliked.id != node.id) {
                            node.id = +node.id;
                            node.x = currentNodeDbCliked.x;
                            node.y = currentNodeDbCliked.y;
                ...
                data.graph.relationships.forEach(function(relationship) {
                    ...
                    relationship.startNode = +relationship.startNode;
                    relationship.endNode = +relationship.endNode;
    ...
    function mergeArrays(nodes, newNodes){
      newNodes.forEach(function(node){
        // filter nodes by id
        var temp = nodes.filter(function(d){
          return d.id == node.id;
        });
        if (temp.length == 0){
          nodes.push(node);
    ...
    function updateNodesAndRelationships(n, r) {
        ...
        if (currentNodeDbCliked) {
          for (var i = 0; i < options.tickSpeed; i++) {
            simulation.tick();
          }
        }
    }
    ...
    function updateRelationships(r) {
        ...
        relationships = mergeArrays(relationships, r);
